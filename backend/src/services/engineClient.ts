...
/**
 * Raw state returned by the Python simulation engine.
 */
interface PythonSimulationState {
  timestamp?: string;
...
export class EngineClient {
  ...
  constructor(
    ...
    this.currentMockState =
      this.generateBaseMockState('com-offgrid-01');

    if (this.isMock) {
      logger.info(
        'EngineClient initialized in MOCK MODE (Python engine simulated)'
      );
    } else {
      logger.info(
        `EngineClient initialized pointing to Python Engine at ${baseURL}`
      );
    }
  }
  ...
  private handleError(
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (
        ...
        logger.error(
          `Python engine connection refused during [${operationName}] at ${env.ENGINE_BASE_URL}`
        );

        throw new EngineUnavailableError(
          `Python engine unreachable at ${env.ENGINE_BASE_URL}`
        );
      }
      ...
      if (
        ...
        logger.error(
          `Python engine request timed out during [${operationName}]`
        );

        throw new EngineTimeoutError(
          `Python engine timed out during ${operationName}`
        );
      }

      if (axiosErr.response) {
        logger.error(
          `Python engine returned error ${axiosErr.response.status} for [${operationName}]`,
          {
        ...
        throw new EngineResponseError(
          `Engine error (${axiosErr.response.status}): ${JSON.stringify(
    ...
    logger.error(
      `Unexpected failure communicating with Python engine in [${operationName}]`,
      error
    );

    throw new EngineUnavailableError(
      `Failed to communicate with optimization engine: ${
  ...
  // ==========================================
  // Python Simulation -> Node Backend Adapter
  // ==========================================

  /**
   * Converts raw Python simulation state into
   * the CommunityState contract expected by Node.
   */
  private normalizeSimulationState(
    raw: PythonSimulationState,
    communityId: string
  ): CommunityState {
    const solarKw =
      raw.solar_output_kw ?? 0;

    const windKw =
      raw.wind_output_kw ?? 0;

    const dieselKw =
      raw.diesel?.current_load_kw ?? 0;
    ...
    const demandTotal =
      raw.demand?.total_kw ?? 0;

    const tier1 =
      raw.demand?.by_tier?.['1'] ?? 0;

    const tier2 =
      raw.demand?.by_tier?.['2'] ?? 0;

    const tier3 =
      raw.demand?.by_tier?.['3'] ?? 0;

    const tier4 =
      raw.demand?.by_tier?.['4'] ?? 0;

    const batterySoc =
      raw.battery?.soc_pct ?? 0;

    const batteryCapacity =
      raw.battery?.capacity_kwh ?? 100;

    const batteryStoredKwh =
      (batterySoc / 100) *
      batteryCapacity;

    const batteryChargeKw =
      raw.battery?.charge_rate_kw ?? 0;

    const dieselFuel =
      raw.diesel?.fuel_remaining_l ?? 0;

    const dieselTank =
      raw.diesel?.tank_capacity_l ?? 2000;

    const dieselFuelPct =
      dieselTank > 0
        ? (dieselFuel / dieselTank) * 100
        : 0;

    const dieselRunning =
      raw.diesel?.generator_on === true ||
      dieselKw > 0;
    ...
    const hasShortfall =
      demandTotal > totalAvailableKw;

    return {
      community_id: communityId,

      name:
        'Kipawa Eco-Community Microgrid',

      timestamp,

      weather: {
        irradiance_w_m2:
          raw.weather?.ghi_w_m2 ?? 0,

        cloud_cover_pct:
          raw.weather?.cloud_cover_pct ?? 0,

        wind_speed_m_s:
          raw.weather?.wind_speed_m_s ?? 0,

        temperature_c:
          raw.weather?.temperature_c ?? 25,

        condition:
          (raw.weather?.ghi_w_m2 ?? 0) > 100
            ? 'SUNNY'
            : 'CLOUDY',

        source:
          raw.weather?.source === 'OPEN_METEO'
            ? 'OPEN_METEO'
            : 'SYNTHETIC'
      },
      ...
      diesel: {
        ...
        optimal_sweet_spot_kw: 80,

        fuel_consumption_rate_l_per_kwh:
          raw.diesel?.consumption_rate_l_per_kwh ??
          0.27,
      ...
      shortfall_stage:
        ShortfallStage.STAGE_0_EARLY_WARNING,
      ...
      signal: {
        ...
        message:
          hasShortfall
            ? 'Microgrid supply below current demand. Monitoring shortfall response.'
            : 'Microgrid operating within available generation.',
  ...
  async getLiveState(
    ...
    try {
      const response =
        await this.client.get<PythonSimulationState>(
          '/state',
          {
            params: {
              community_id: communityId
      ...
      return this.normalizeSimulationState(
        response.data,
        communityId
      );
    } catch (err) {
  ...
  private generateBaseMockState(
    ...
    let signalMsg =
      'Microgrid operating on 100% renewable generation. Battery charging.';
    ...
    if (net >= 0) {
      ...
      if (deficit <= 50.0) {
        ...
        signalMsg =
          'Solar & Wind supported by Battery storage. Zero emissions.';
      } else {
        dieselKw = 80.0;
        ...
        shortfallStage =
          ShortfallStage.STAGE_3_EFFICIENT_DIESEL;

        signalColor =
          'YELLOW';

        signalMsg =
          'Efficient Diesel active at 80 kW sweet spot to preserve grid stability.';
      }
    }
  ...
  private getMockLiveState(
    communityId: string
  ): CommunityState {
    this.currentMockState =
      this.generateBaseMockState(
        communityId
      );
  ...
  private getMockDispatchPlan(
    ...
    let currentSoc =
      payload.current_state?.battery.soc_pct ??
      80.0;
  ...
  private getMockLadderResponse(
    ...
    const actions = [
      ...
      stage >= 1
        ? 'Pre-charging battery bank from surplus generation'
        : 'Normal dispatch monitoring',

      stage >= 2
        ? 'Rescheduled Tier 4 EV and heavy milling loads'
        : 'Tier 4 loads active',

      stage >= 3
        ? 'Dispatched generator at 80 kW sweet spot, routing 16 kW surplus into battery'
        : 'Diesel OFF',

      stage === 4
        ? 'Emergency curtailment of Tier 4 and partial Tier 3 loads'
        : 'No loads shed'
    ];
  ...
  private getMockRunwayForecast(
    ...
    for (
      ...
      daily.push({
        ...
        projected_diesel_liters_burned:
          Math.round(
            burned *
            10
          ) / 10,
  ...
  private getMockScenarioResult(
    ...
    let message =
      `Applied ${payload.name}`;

    if (
      payload.scenario_type ===
      'WIND_DROP'
    ) {
      updated.generation.wind_kw =
        Math.round(
          updated.generation.wind_kw *
          0.2 *
          10
      ...
      updated.generation.diesel_kw =
        80.0;

      updated.shortfall_stage =
        ShortfallStage.STAGE_3_EFFICIENT_DIESEL;
      ...
      updated.signal = {
        ...
        message:
          'Wind drop detected: Diesel dispatched at 80 kW sweet spot to preserve stability.',
      ...
      message =
        'Wind output collapsed by 80%. Diesel started at 80 kW sweet spot. 18 kW excess routed to battery.';
    } else if (
      payload.scenario_type ===
      'CLOUD_COVER'
    ) {
      updated.generation.solar_kw =
        Math.round(
          updated.generation.solar_kw *
          0.15 *
          10
      ...
      updated.battery.current_discharge_kw =
        35.0;

      updated.shortfall_stage =
        ShortfallStage.STAGE_1_PREEMPTIVE_PRECHARGE;
      ...
      message =
        'Heavy cloud cover reduced solar by 85%. Battery discharge stepped up smoothly.';
    } else if (
      payload.scenario_type ===
      'BATTERY_FAULT'
    ) {
      updated.battery.health_pct =
        0;

      updated.battery.current_discharge_kw =
        0;

      updated.battery.current_charge_kw =
        0;
      ...
      updated.signal = {
        ...
        message:
          'Battery fault isolated. Diesel generator providing continuous voltage stabilization.',
      ...
      message =
        'Battery storage faulted and safely isolated. Diesel generator active.';
    }
    ...
    return {
      ...
      reoptimized_dispatch:
        this.getMockDispatchPlan({
          horizon_hours: 24
        }),

      ladder_response:
        this.getMockLadderResponse(
          updated.shortfall_stage
        ),
